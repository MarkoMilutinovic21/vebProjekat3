...
namespace TravelPlanService
{
    internal sealed class TravelPlanService : StatefulService, ITravelPlanService
    {
        ...
        private static TravelPlanDto MapToDto(TravelPlan plan) => new TravelPlanDto
        {
            ...
            TotalExpenses = (plan.Expenses?.Sum(e => e.Amount) ?? 0) + (plan.Activities?.Sum(a => a.EstimatedCost) ?? 0),
            RemainingBudget = plan.Budget - ((plan.Expenses?.Sum(e => e.Amount) ?? 0) + (plan.Activities?.Sum(a => a.EstimatedCost) ?? 0)),
